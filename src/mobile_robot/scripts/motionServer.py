...
def callback(data):
	...
	if(flag==0):
		...
		if(abs(cur_bear-targets[n])%360<45):
			isRotate=False
		else: 
			isRotate=True
	global c

	if(abs(cur_bear-targets[n])%360<5 and distance(xc,yc,xt,yt)<=0.05):
		stop()
                rospy.loginfo(str(c))
                global rotateSubscribe
	...
	if(isRotate):
		...
		#target=targets[n]
               	if(not decideDir(cur_bear,targets[n])):
                        ...
                        right(12,17)
		if(abs(cur_bear-targets[n])%360<=5):
		...
		#rospy.loginfo("else case %d\n",flag)

		if(abs(cur_bear-target)%360>5 and distance(xc,yc,xt,yt)>0.05):
