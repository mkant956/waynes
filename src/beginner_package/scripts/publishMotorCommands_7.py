...
def callback(bear_data,encoder_data):
    ...
    if(abs(cur_encd-initial_encd)>tomove):
        # global bear_data
    #     global encoder_data
        rospy.loginfo(bear_data)
        # subprocess.call()
        #os.system("a=$(rosnode list | awk -F' ' '{print$1}')")
        # os.system("a=$(rosnode list | grep left_encod)")
        os.system("rosnode kill $(rosnode list | grep left_encod)")
        # os.system('rosnode kill $a')
        os.system('rosrun beginner_package publishEncoderl.py &')
